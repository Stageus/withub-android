package com.example.withub.feature.drawer
...
import com.example.withub.data.network.FriendApi
import com.example.withub.data.network.FriendNameData
...
import com.example.withub.feature.setting.SettingActivity
import kotlinx.coroutines.*

class DrawerActivity : AppCompatActivity() {

    private lateinit var binding: ActivityDrawerBinding
    private lateinit var navFriendRVAdapter: DrawerFriendListRVAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CoroutineScope(Dispatchers.Main).launch() {
            ...
            binding.drawerFriendRecyclerView.apply {
                ...
                adapter = DrawerFriendListRVAdapter(
                    this@DrawerActivity,
                    getFriendList.await().toMutableList(),
                    job.await()
                )
            }
                    ...
                    .setPositiveButton("추가") { _, _ ->
                        CoroutineScope(Dispatchers.Main).launch(handler) {
                            ...
                            if (addFriendToList.await().success) {
                                ...
                                navFriendRVAdapter.addItem(
                                    friendList[friendList.lastIndex],
                                    friendList.size
                                )
                            } else {
                                val failDialog: AlertDialog.Builder =
                                    AlertDialog.Builder(this@DrawerActivity)
